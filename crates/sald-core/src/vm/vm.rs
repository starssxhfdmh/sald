...
fn op_set_local(vm: &mut VM) -> ControlFlow {
    ...
    if !vm.stack.is_empty() {
        ...
        unsafe {
            *vm.stack.get_unchecked_mut(slots_start + slot) = value;
        }
    }
    ControlFlow::Continue
}
        ...
        (Value::Number(av), Value::Number(bv)) => {
            if *bv == 0.0 {
                return ControlFlow::Error(
                    vm.create_error(ErrorKind::DivisionByZero, "Division by zero"),
                );
            }
            let result = av / bv;
            unsafe {
...
fn op_mod(vm: &mut VM) -> ControlFlow {
    ...
    match (a, b) {
        (Value::Number(av), Value::Number(bv)) => {
            if *bv == 0.0 {
                return ControlFlow::Error(
                    vm.create_error(ErrorKind::DivisionByZero, "Modulo by zero"),
                );
            }
            let result = av % bv;
            unsafe {
...
fn op_negate(vm: &mut VM) -> ControlFlow {
    ...
    match v {
        ...
        _ => {
            ...
            ControlFlow::Error(
                vm.create_error(ErrorKind::TypeError, &format!("Cannot negate '{}'", v_type)),
            )
        }
    }
}
...
fn op_import_as(vm: &mut VM) -> ControlFlow {
    ...
    match (
        vm.read_string_constant(path_idx),
        vm.read_string_constant(alias_idx),
    ) {
...
fn op_await(vm: &mut VM) -> ControlFlow {
    ...
    match value {
        Value::Future(_) => ControlFlow::Error(vm.create_error(
            ErrorKind::RuntimeError,
            "async/await is not supported in WASM playground",
        )),
        other => {
            vm.stack.push(other);
            ControlFlow::Continue
...
impl VM {
    ...
    fn execute_one_threaded(&mut self) -> ControlFlow {
        ...
        if op < 69 {
            ...
            ControlFlow::Error(
                self.create_error(ErrorKind::RuntimeError, &format!("Unknown opcode: {}", op)),
            )
        }
    }
    ...
    fn call_function(&mut self, function: Arc<Function>, arg_count: usize) -> SaldResult<()> {
        ...
        for _ in 0..variadic_count {
            variadic_args.push(self.stack.pop().unwrap_or(Value::Null));
        }
    ...
    fn call_instance_method(
        &mut self,
        receiver: Value,
        method: fn(&Value, &[Value]) -> Result<Value, String>,
        arg_count: usize,
    ) -> SaldResult<()> {
        let args: Vec<Value> = self.stack.drain(self.stack.len() - arg_count..).collect();
        self.stack.pop();
    ...
    fn call_function_init_with_class(
        ...
        for _ in 0..(function.arity - arg_count) {
            self.stack.push(Value::Null);
        }
        if self.frames.len() >= FRAMES_MAX {
            return Err(self.create_error(
        ...
        self.frames.push(CallFrame::new_init_with_class(
            function,
            slots_start,
            instance,
            class_name,
            ...
            Value::Instance(ref instance) => {
                ...
                if Self::is_private(name) && !self.is_in_class(&class.name) {
                    return Err(self.create_error(
                        ...
                        &format!(
                            "Cannot access private method '{}' from outside class '{}'",
                            name, class.name
                        ),
                    ));
                }
                ...
                Err(self.create_error(
                    ...
                    &format!("Undefined method '{}' on instance", name),
                ))
            }
            Value::Class(class) => {
                ...
                if Self::is_private(name) && !self.is_in_class(&class.name) {
                    return Err(self.create_error(
                        ErrorKind::AccessError,
                        &format!(
                            "Cannot access private static method '{}' from outside class '{}'",
                            name, class.name
                        ),
                    ));
                }
            ...
            | Value::Dictionary(_) => {
                ...
                let class =
                    if let Some(Value::Class(c)) = self.globals.read().get(class_name).cloned() {
                        ...
                        return Err(self.create_error(
                            ErrorKind::RuntimeError,
                            &format!("Built-in class '{}' not found", class_name),
                        ));
                    };
                if let Some(callable_method) =
                    class.callable_native_instance_methods.get(name).copied()
            ...
            Value::Namespace {
                ...
                if Self::is_private(name) && !self.is_in_namespace(&ns_name) {
                    return Err(self.create_error(
                        ErrorKind::AccessError,
                        &format!(
                            "Cannot access private member '{}' from outside namespace '{}'",
                            name, ns_name
                        ),
                    ));
                }
    ...
    fn handle_get_property(&mut self, name: &str) -> SaldResult<()> {
        ...
        match obj {
            ...
            | Value::Dictionary(_) => {
                ...
                match method_result {
                    Ok((method_fn, method_name)) => {
                        self.stack.push(Value::InstanceMethod {
            ...
            _ => {
                return Err(self.create_error(
                    ErrorKind::TypeError,
                    &format!("Only instances have properties, got '{}'", obj.type_name()),
                ))
            }
        }
        Ok(())
        ...
        for (key, value) in pairs {
            if let (Value::Null, Value::SpreadMarker(spread_value)) = (&key, &value) {
                if let Value::Dictionary(dict) = spread_value.as_ref() {
                    ...
                    return Err(self.create_error(
                        ErrorKind::TypeError,
                        "Can only spread dictionaries with **",
                    ));
                }
            } else {
                let key_str = match key {
                    ...
                    _ => {
                        return Err(self
                            .create_error(ErrorKind::TypeError, "Dictionary keys must be strings"))
                    }
                };
    ...
    fn handle_build_namespace(&mut self) -> SaldResult<()> {
        let count = self.read_u16() as usize;
        let mut members: FxHashMap<String, Value> =
            FxHashMap::with_capacity_and_hasher(count, Default::default());
        for _ in 0..count {
            ...
            if let Value::String(s) = key {
                ...
                return Err(self.create_error(
                    ErrorKind::TypeError,
                    "Namespace member keys must be strings",
                ));
            }
        }
    ...
    fn handle_build_enum(&mut self) -> SaldResult<()> {
        let count = self.read_u16() as usize;
        let mut variants: FxHashMap<String, Value> =
            FxHashMap::with_capacity_and_hasher(count, Default::default());
        for _ in 0..count {
            let value = self.stack.pop().unwrap_or(Value::Null);
            let key = self.stack.pop().unwrap_or(Value::Null);
            if let Value::String(s) = key {
                variants.insert(s.to_string(), value);
            } else {
                return Err(
                    self.create_error(ErrorKind::TypeError, "Enum variant keys must be strings")
                );
            }
        }
    ...
    fn handle_build_range(&mut self, inclusive: bool) -> SaldResult<()> {
        ...
        let start_num = match &start {
            ...
            _ => {
                return Err(self.create_error(
                    ErrorKind::TypeError,
                    &format!("Range start must be a number, got {}", start.type_name()),
                ))
            }
        };
        let end_num = match &end {
            ...
            _ => {
                return Err(self.create_error(
                    ErrorKind::TypeError,
                    &format!("Range end must be a number, got {}", end.type_name()),
                ))
            }
        };
    ...
    fn handle_get_super(&mut self, method_name: &str) -> SaldResult<()> {
        ...
        if let Value::Instance(ref instance) = receiver {
            ...
            if let Some(ref superclass) = class.superclass {
                if let Some(method) = superclass.methods.get(method_name) {
                    ...
                    return Err(self.create_error(
                        ErrorKind::TypeError,
                        &format!("Method '{}' is not a function", method_name),
                    ));
                }
                return Err(self.create_error(
                    ErrorKind::AttributeError,
                    &format!("Undefined method '{}' in superclass", method_name),
                ));
            }
        ...
        Err(self.create_error(
            ErrorKind::RuntimeError,
            "'super' can only be used in instance methods",
        ))
    }
    ...
    fn handle_import(&mut self, import_path: &str) -> SaldResult<()> {
        Err(self.create_error(
            ...
            &format!(
                "import is not supported in WASM playground: {}",
                import_path
            ),
        ))
    }
    ...
    fn is_module_import(path: &str) -> bool {
        !path.contains('/')
            && !path.contains('\\')
            && !path.ends_with(".sald")
            && !path.ends_with(".saldc")
    }

    #[cfg(not(target_arch = "wasm32"))]
    fn resolve_module_import(&mut self, module_name: &str) -> SaldResult<String> {
        let project_root = crate::get_project_root().ok_or_else(|| {
            self.create_error(
                ErrorKind::ImportError,
                &format!(
                    "Cannot import module '{}': no project root set",
                    module_name
                ),
            )
        })?;
        ...
        if !main_path.exists() {
            return Err(self.create_error(
                ErrorKind::ImportError,
                &format!(
                    "Module '{}' main file '{}' not found",
                    module_name, main_entry
                ),
            ));
        }
        self.pending_module_workspace = Some(module_dir);
        main_path.to_str().map(|s| s.to_string()).ok_or_else(|| {
            self.create_error(
                ErrorKind::ImportError,
    ...
    fn resolve_file_import(&self, import_path: &str) -> SaldResult<String> {
        ...
        if path_buf.is_absolute() && path_buf.exists() {
            return canonicalize(path_buf.clone()).ok_or_else(|| {
                self.create_error(
                    ErrorKind::ImportError,
        ...
        if relative_path.exists() {
            return canonicalize(relative_path.clone()).ok_or_else(|| {
                self.create_error(
                    ErrorKind::ImportError,
                    &format!("Invalid import path: {}", import_path),
                )
        ...
        if let Ok(module_path) = env::var("SALD_MODULE") {
            ...
            if env_path.exists() {
                return canonicalize(env_path.clone()).ok_or_else(|| {
                    self.create_error(
                        ErrorKind::ImportError,
                        &format!("Invalid import path: {}", import_path),
                    )
    ...
    fn import_and_execute(&mut self, path: &str) -> SaldResult<FxHashMap<String, Value>> {
        let chunk = if path.ends_with(".saldc") {
            let data = std::fs::read(path).map_err(|e| {
                self.create_error(
                    ErrorKind::ImportError,
                    &format!("Cannot read import file '{}': {}", path, e),
                )
            })?;
            crate::binary::deserialize(&data).map_err(|e| {
                self.create_error(
                    ErrorKind::ImportError,
                    &format!("Error deserializing import '{}': {}", path, e),
                )
            })?
        } else {
            ...
            let tokens = scanner.scan_tokens().map_err(|e| {
                self.create_error(
                    ...
                    &format!("Error scanning import '{}': {}", path, e),
                )
            })?;
            ...
            let program = parser.parse().map_err(|e| {
                self.create_error(
                    ErrorKind::SyntaxError,
                    &format!("Error parsing import '{}': {}", path, e),
                )
            })?;
            ...
            compiler.compile(&program).map_err(|e| {
                self.create_error(
                    ErrorKind::SyntaxError,
                    &format!("Error compiling import '{}': {}", path, e),
                )
            })?
        };
