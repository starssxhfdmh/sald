...
fn server_listen(
    ...
    if routes.is_empty() {
        return Err(
            "No routes registered. Add routes with server.get(), server.post(), etc.".to_string(),
        );
    }
    ...
    tokio::task::block_in_place(|| {
        tokio::runtime::Handle::current()
            .block_on(async move { run_async_server(port, routes, globals, script_dir_str).await })
    })
}
    ...
    loop {
        match listener.accept().await {
            Ok((stream, _addr)) => {
                ...
                tokio::spawn(async move {
                    if let Err(e) =
                        handle_connection(stream, routes_clone, globals_clone, script_dir_clone)
                            .await
                    {
...
async fn handle_connection(
    ...
    loop {
        ...
        let mut headers: FxHashMap<String, Value> =
        ...
        let mut query_params: FxHashMap<String, Value> =
        ...
        if !query_string.is_empty() {
            for pair in query_string.split('&') {
                if let Some(eq_idx) = pair.find('=') {
                    ...
                    query_params
                        .insert(key.to_string(), Value::String(Arc::from(value.to_string())));
                }
            }
        }
        ...
        if let Some(handler_fn) = handler {
            ...
            let mut req_fields: FxHashMap<String, Value> =
                FxHashMap::with_capacity_and_hasher(6, Default::default());
...
fn find_matching_route(
    ...
    let mut params: FxHashMap<String, Value> =
        FxHashMap::with_capacity_and_hasher(4, Default::default());
        ...
        Value::Dictionary(dict) => {
            ...
            let headers: FxHashMap<String, String> =
                if let Some(Value::Dictionary(hdrs)) = dict_guard.get("headers") {
                    ...
                    let mut h =
                        FxHashMap::with_capacity_and_hasher(hdrs_guard.len(), Default::default());
