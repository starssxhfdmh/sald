...
fn system_info(_args: &[Value]) -> Result<Value, String> {
    ...
    info.insert(
        "arch".to_string(),
        Value::String(Rc::from(std::env::consts::ARCH.to_string())),
    );
    info.insert(
        "family".to_string(),
        Value::String(Rc::from(std::env::consts::FAMILY.to_string())),
    );
    info.insert(
        "cpus".to_string(),
        Value::Number(
    ...
    info.insert(
        "usedMemoryMB".to_string(),
        Value::Number((sys.used_memory() / 1024 / 1024) as f64),
    );
    info.insert(
        "freeMemoryMB".to_string(),
...
fn system_setenv(args: &[Value]) -> Result<Value, String> {
    ...
    let value = match &args[1] {
        ...
        other => {
            return Err(format!(
                "Argument 'value' must be a string, got {}",
                other.type_name()
            ))
        }
    };
...
fn system_envs(_args: &[Value]) -> Result<Value, String> {
    ...
    for (key, value) in std::env::vars() {
        envs.insert(key, Value::String(Rc::from(value)));
    }

    Ok(Value::Dictionary(Rc::new(RefCell::new(envs))))
}
