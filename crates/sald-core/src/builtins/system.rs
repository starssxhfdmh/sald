...
fn system_info(_args: &[Value]) -> Result<Value, String> {
    ...
    info.insert(
        "os".to_string(),
        Value::String(Arc::from(std::env::consts::OS.to_string())),
    );
    info.insert(
        "arch".to_string(),
        Value::String(Arc::from(std::env::consts::ARCH.to_string())),
    );
    info.insert(
        "family".to_string(),
        Value::String(Arc::from(std::env::consts::FAMILY.to_string())),
    );
    info.insert(
        "cpus".to_string(),
        Value::Number(
