...
async fn repl() -> Result<(), String> {
    ...
    let history = Box::new(
        FileBackedHistory::with_file(1000, history_path.clone()).map_err(|e| e.to_string())?,
    );
    ...
    match value {
        ...
        Value::Array(arr) => {
            ...
            if arr.is_empty() {
                ...
                let items: Vec<String> = arr.iter().map(|v| format_value(v, depth + 1)).collect();
                format!("[ {} ]", items.join(", "))
            } else {
        ...
        Value::Dictionary(dict) => {
            ...
            if dict.is_empty() {
                ...
                format!("{{Object({} keys)}}", dict.len())
                    .bright_black()
                    .to_string()
            }
        }
        Value::Function(f) => format!("[Function: {}]", f.name).cyan().to_string(),
        Value::NativeFunction { class_name, .. } => format!("[NativeFunction: {}]", class_name)
            .cyan()
            .to_string(),
        ...
        Stmt::LetDestructure {
            ...
            let vars: Vec<String> = pattern
                ...
                .map(|e| match e {
                    ...
                    sald_core::ast::ArrayPatternElement::Rest { name, .. } => {
                        format!("...{}", name)
                    }
...
fn build_pattern_tree(tree: &mut ptree::TreeBuilder, pattern: &sald_core::ast::Pattern) {
    use sald_core::ast::{Literal, Pattern, SwitchArrayElement};
