...
use sald_core::ast::{ClassDef, Expr, FunctionDef, Stmt};
...
impl SaldLanguageServer {
    ...
    fn error_to_diagnostic(&self, e: &sald_core::error::SaldError) -> Diagnostic {
        Diagnostic {
    ...
    fn extract_expr_symbols(&self, expr: &Expr, symbols: &mut Vec<Symbol>) {
        match expr {
            Expr::Lambda {
                ...
                match body {
                    sald_core::ast::LambdaBody::Expr(e) => self.extract_expr_symbols(e, symbols),
                    sald_core::ast::LambdaBody::Block(stmts) => {
                        for stmt in stmts {
    ...
    fn extract_properties_from_stmts(
        &self,
        stmts: &[Stmt],
            ...
            if let Some(line) = lines.get(position.line as usize) {
                ...
                if let Some(dot_pos) = prefix.rfind('.') {
                    ...
                    if let Some(type_name) = self.find_variable_type(&doc.symbols, var_name) {
                        ...
                        if let Some(type_sym) = self
                            .resolve_type_path(&doc.symbols, &type_name)
                    ...
                    if let Some(sym) = self
                        .find_symbol_by_name(&doc.symbols, var_name)
    ...
    fn find_class_at_position<'a>(
        ...
        for sym in symbols {
            if sym.kind == SymbolKind::Class {
                ...
                if position.line >= sym.range.start.line && position.line <= sym.range.end.line {
                    return Some(sym);
                }
            }
            ...
            if let Some(ref source_path) = sym.source_uri {
                if !source_path.is_empty() {
                    ...
                    if let Ok(file_uri) = Url::from_file_path(path) {
                        ...
                        if sym.selection_range.end.line > 0
                            || sym.selection_range.end.character > 0
    ...
    fn get_hover_info(&self, uri: &Url, position: Position) -> Option<Hover> {
        ...
        if let Some((string_content, string_start, string_end)) =
            self.find_string_at_position(line, char_pos)
        {
            let word_range = Range {
                start: Position {
                    line: position.line,
                    character: string_start as u32,
                },
                end: Position {
                    line: position.line,
                    character: string_end as u32,
                },
            };
    ...
    fn get_keyword_hover(&self, word: &str, range: &Range) -> Option<Hover> {
        let (keyword, description, example) = match word {
            ...
            "async" => (
                "async",
                "Declares an async function that returns a Future.",
                "async fun fetch(url) { ... }",
            ),
            "await" => (
                "await",
                "Waits for an async expression to complete.",
                "let data = await fetch(url)",
            ),
            "class" => (
                "class",
                "Declares a class with methods and properties.",
                "class Person { ... }",
            ),
            "extends" => (
                "extends",
            ...
            "in" => (
                "in",
                "Iterator keyword used in for loops.",
                "for x in array { ... }",
            ),
            "do" => (
                "do",
    ...
    fn format_symbol_hover(&self, sym: &Symbol, range: &Range, is_builtin: bool) -> Hover {
        ...
        let code_block = match sym.kind {
            ...
            SymbolKind::Function => sym
                .detail
                .clone()
                .unwrap_or_else(|| format!("fun {}()", sym.name)),
    ...
    async fn index_workspace(&self) {
        ...
        self.client
            .log_message(
                MessageType::INFO,
                format!("Indexing {} workspace files...", files.len()),
            )
            .await;
        ...
        self.client
            .log_message(
                MessageType::INFO,
                format!(
                    "Analyzing {} project files for diagnostics...",
                    analyze_files.len()
                ),
            )
            .await;
    ...
    fn index_file(&self, file_path: &std::path::Path) {
        ...
        self.workspace_index
            .register_exports(file_path.to_path_buf(), exports);
        ...
        // Register references
        self.workspace_index
            .register_references(&file_path.to_path_buf(), references);
    }
    ...
    fn collect_references_from_expr(&self, expr: &Expr, refs: &mut Vec<String>) {
        match expr {
            ...
            Expr::Lambda { body, .. } => match body {
                sald_core::ast::LambdaBody::Expr(e) => self.collect_references_from_expr(e, refs),
                sald_core::ast::LambdaBody::Block(stmts) => {
                    for s in stmts {
                        self.collect_references_from_stmt(s, refs);
                    }
        ...
        if let Some(root_uri) = params.root_uri {
            if let Ok(path) = root_uri.to_file_path() {
                self.import_resolver
                    .write()
                    .set_workspace_root(path.clone());
                self.workspace_index.set_workspace_root(path.clone());
                sald_core::set_project_root(&path);
