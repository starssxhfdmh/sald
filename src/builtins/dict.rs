    ...
    if args.is_empty() {
        ...
        match &args[0] {
            Value::Dictionary(source) => {
                let copy = source.lock().unwrap().clone();
...
fn dict_length(recv: &Value, args: &[Value]) -> Result<Value, String> {
    ...
    match recv {
        Value::Dictionary(dict) => Ok(Value::Number(dict.lock().unwrap().len() as f64)),
...
fn dict_values(recv: &Value, args: &[Value]) -> Result<Value, String> {
    ...
    match recv {
        Value::Dictionary(dict) => {
            let values: Vec<Value> = dict.lock().unwrap().values().cloned().collect();
...
fn dict_set(recv: &Value, args: &[Value]) -> Result<Value, String> {
    ...
    match recv {
        Value::Dictionary(dict) => {
            let key = get_string_arg(&args[0], "key")?;
            dict.lock().unwrap().insert(key, args[1].clone());
            Ok(Value::Null)
        }
        _ => Err("Receiver must be a dictionary".to_string()),
    }
...
fn dict_has(recv: &Value, args: &[Value]) -> Result<Value, String> {
    ...
    match recv {
        Value::Dictionary(dict) => {
            let key = get_string_arg(&args[0], "key")?;
            Ok(Value::Boolean(dict.lock().unwrap().contains_key(&key)))
        }
        _ => Err("Receiver must be a dictionary".to_string()),
    }
}
...
fn dict_remove(recv: &Value, args: &[Value]) -> Result<Value, String> {
    ...
    match recv {
        Value::Dictionary(dict) => {
            let key = get_string_arg(&args[0], "key")?;
            Ok(dict.lock().unwrap().remove(&key).unwrap_or(Value::Null))
        }
        _ => Err("Receiver must be a dictionary".to_string()),
    }
}
...
fn dict_is_empty(recv: &Value, args: &[Value]) -> Result<Value, String> {
    check_arity(0, args.len())?;
    match recv {
        Value::Dictionary(dict) => Ok(Value::Boolean(dict.lock().unwrap().is_empty())),
        _ => Err("Receiver must be a dictionary".to_string()),
    }
}
