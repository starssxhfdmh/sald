...
fn promise_all(args: &[Value]) -> Result<Value, String> {
    ...
    if let Ok(handle) = tokio::runtime::Handle::try_current() {
        ...
        let mut item_receivers: Vec<oneshot::Receiver<Result<Value, String>>> = Vec::with_capacity(futures.len());
                    ...
                    if let Some(sald_future) = receiver {
                        ...
                        handle.spawn(async move {
                            match sald_future.receiver.await {
                                Ok(Ok(value)) => { let _ = item_tx.send(Ok(value)); }
                                Ok(Err(e)) => { let _ = item_tx.send(Err(e)); }
                                Err(_) => { let _ = item_tx.send(Err("Future was cancelled".to_string())); }
                            }
                        });
                    } else {
